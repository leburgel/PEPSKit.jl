...
function fixed_point_2_transfer(
    ...
    return fixed_point_2_transfer(Val(Gauge), Val(Style), A, A, A, X, X, X, X, O)
end
...
function fixed_point_2_transfer(
    ...
    return fixed_point_2_transfer_naive(AN, AW, AS, XNW, XSW, O)
end
function fixed_point_2_transfer(
    ::Val{Gauge},
    ::Val{:regularized},
    ...
    return fixed_point_2_transfer_regularized(AN, AW, AS, XNW, XSW, XNE, XSE, O)
end

function fixed_point_2_transfer_naive(
...
function fixed_point_2_transfer_regularized(
    AN::MPSKit.GenericMPSTensor{S},
    AW::MPSKit.GenericMPSTensor{S},
    AS::MPSKit.GenericMPSTensor{S},
    XNW::MPSKit.MPSBondTensor{S},
    ...
    AW´ = fixed_point_2_transfer_naive(AN, AW, AS, XNW, XSW, O)
    return absorb_bond_matrices(AW´, inv(XSE), inv(XNE))
end
...
function fixed_point_2(
    ...
    FP2 =
        fixed_point_2_transfer(Val(Gauge), Val(:naive), A, X, O) -
        fixed_point_2_vector(Val(Gauge), Val(:naive), A, X, N)
    return FP2
end
...
function fixed_point_1_pushforward(
    ...
    @warn "This one is completely broken for now"
    ∂FP1 =
...
function fixed_point_2_pushforward(
    ...
    ∂FP2 =
        fixed_point_2_transfer_naive(∂A, A, A, X, X, O) +
        fixed_point_2_transfer_naive(A, ∂A, A, X, X, O) +
        fixed_point_2_transfer_naive(A, A, ∂A, X, X, O) +
        fixed_point_2_transfer_naive(A, A, A, ∂X, X, O) +
        fixed_point_2_transfer_naive(A, A, A, X, ∂X, O) -
        fixed_point_2_vector(Val(Gauge), Val(:naive), ∂A, X, X, N) -
        fixed_point_2_vector(Val(Gauge), Val(:naive), A, ∂X, X, N) -
        fixed_point_2_vector(Val(Gauge), Val(:naive), A, X, ∂X, N) -
        fixed_point_2_vector(Val(Gauge), Val(:naive), A, X, X, ∂N)
    return ∂FP2
...
function fixed_point_2_pushforward(
    ...
    @warn "This one is completely broken for now"
    blabla = inv(X) * ∂X * inv(X)
    ∂FP2 =
        fixed_point_2_transfer_regularized(∂A, A, A, inv(X), X, inv(X), X, O) +
        fixed_point_2_transfer_regularized(A, ∂A, A, inv(X), inv(X), X, X, O) +
        fixed_point_2_transfer_regularized(A, A, ∂A, X, inv(X), X, inv(X), O) +
        fixed_point_2_transfer_regularized(A, A, A, blabla, X, X, X, O) +
        fixed_point_2_transfer_regularized(A, A, A, X, blabla, X, X, O) +
        fixed_point_2_transfer_regularized(A, A, A, X, X, blabla, X, O) +
        fixed_point_2_transfer_regularized(A, A, A, X, X, X, blabla, O) -
        fixed_point_2_vector(Val(Gauge), Val(:naive), ∂A, inv(X), inv(X), N) -
        fixed_point_2_vector(Val(Gauge), Val(:naive), A, X, X, ∂N)
    return ∂FP2
end
...
function fixed_point_3_pushforward(
    ...
    ∂FP3 =
        fixed_point_3_transfer(Val(:center), Val(:naive), ∂A, A, X, X) +
        fixed_point_3_transfer(Val(:center), Val(:naive), A, ∂A, X, X) +
        fixed_point_3_transfer(Val(:center), Val(:naive), A, A, ∂X, X) +
        fixed_point_3_transfer(Val(:center), Val(:naive), A, A, X, ∂X) -
        fixed_point_3_vector(Val(:center), Val(:naive), ∂X, X) -
        fixed_point_3_vector(Val(:center), Val(:naive), X, ∂X)
    return ∂FP3
end
function fixed_point_3_pushforward(
    ...
    @warn "Completely broken for now"
    blabla = inv(X) * ∂X * inv(X)
    ∂FP3 =
        fixed_point_3_transfer(Val(:center), Val(:regularized), ∂A * inv(X), A, inv(X), X) +
        fixed_point_3_transfer(Val(:center), Val(:regularized), A, ∂A * inv(X), X, inv(X)) +
        fixed_point_3_transfer(Val(:center), Val(:regularized), A, A, blabla, X) +
        fixed_point_3_transfer(Val(:center), Val(:regularized), A, A, X, blabla) -
        fixed_point_3_vector(Val(:center), Val(:regularized), blabla, X) -
        fixed_point_3_vector(Val(:center), Val(:regularized), X, blabla)
    return ∂FP3
end
function fixed_point_3_pushforward(
    ::Val{:left},
...
# appropriate (co)tangent projections
function project_hermitian(A::MPSKit.GenericMPSTensor)
    A´ = (A + physical_flip(_conj(A))) / 2
    return A´
end
function project_hermitian(X::MPSKit.MPSBondTensor)
    X´ = (X + X') / 2
    return X´
end

# appropriate tangent preconditioning
function precondition_environment_tangent(
    ::Val{:naive},
    Xfp::MPSKit.MPSBondTensor,
    (∂A, ∂X, ∂N)::Tuple{<:MPSKit.GenericMPSTensor,<:MPSKit.MPSBondTensor,<:Number},
)
    return (∂A, ∂X, ∂N)
end
function precondition_environment_tangent(
    ...
    ∂A = absorb_bond_matrices(∂A, Xprec, Xprec)
    ∂X = ∂X * Xprec / 4

    return (∂A, ∂X, ∂N)
end

function generate_environment_pushforward(
    ::Val{Gauge},
    ::Val{Style},
    ...
    function environment_pushforward(
        (∂A, ∂X, ∂N)::Tuple{TA,TX,TN}
    ) where {TA<:MPSKit.GenericMPSTensor,TX<:MPSKit.MPSBondTensor,TN<:Number}
        if Gauge == :center
            # toggle Hermitian projections
            ∂A = project_hermitian(∂A)
            ∂X = project_hermitian(∂X)
            if X isa DiagonalTensorMap && scalartype(X) <: Real
                # toggle real diagonal projection of corner cotangent
                ∂X = DiagonalTensorMap(∂X) # TODO: actually make this real...
            end
        end

        # precondition tangent vectors...
        (∂A, ∂X, ∂N) = precondition_environment_tangent(Val(Style), X, (∂A, ∂X, ∂N))

        # # Hermiticity -> removed in favor of explicitly projection
        # ∂FP1 = fixed_point_1_pushforward(Val(Gauge), Val(Style), A, X, ∂A, ∂X)

        # Eigenvalue equation
        ∂FP2 = fixed_point_2_pushforward(Val(Gauge), Val(:naive), A, X, N, O, ∂A, ∂X, ∂N)

        # Left fixed point condition
        ∂FP3 = fixed_point_3_pushforward(Val(Gauge), Val(:naive), A, X, ∂A, ∂X)

        # Normalization
        ∂FP4 = fixed_point_4_pushforward(Val(Gauge), Val(:naive), X, ∂X)

        if Gauge == :center
            # toggle Hermitian projections
            ∂FP2 = project_hermitian(∂FP2)
            ∂FP3 = project_hermitian(∂FP3)
        end
    ...
    # preconditioned pushforward

    return environment_pushforward
end
...
# function generate_environment_pullback(::Val{:center}, ::Val{:naive}, network, A, X, N)
#     O = network[1, 1]

#     function environment_pullback((ΔFP1, ΔFP2, ΔFP3, ΔFP4))
#         # project output cotangents onto Hermitian components
